...
straightforward: :math:`\mathrm{PFN} - \mathrm{ARCH\_PFN\_OFFSET}`
is an index to the ``mem_map`` array.
...
.. math::
   ...
   \mathrm{NR\_MEM\_SECTIONS} = 2^{(\mathrm{MAX\_PHYSMEM\_BITS} - \mathrm{SECTION\_SIZE\_BITS})}
