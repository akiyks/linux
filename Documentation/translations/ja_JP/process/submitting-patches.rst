...
.. note:: 【訳註】
   ``Signed-off-by`` によって同意する対象は、以下に示す
   Developer's Certificate of Origin 1.1 の英語原文です。
   DCO は法的な性質を持つ文書であり、公式の翻訳が存在しないため、
   原文をそのまま掲載します。
   内容を確認する場合は、必ず原文を参照してください。
...
作者の SoB (``Signed-off-by:``) に続く追加の SoB は、パッチの開発には
関与せず、その取り扱いや転送を行った人によるものです。SoB の連鎖は、
単独の主要作者を示す最初の SoB から始まり、パッチがメンテナーを経て最終的に
Linus へ届いた\ **実際の**\ 経路を反映することになります。

.. note:: 【訳補】

   DCO の参考訳（2007年に投稿された Documentation/ja_JP/SubmittingPatches による。
   タイトルを含めて非公式） ::
